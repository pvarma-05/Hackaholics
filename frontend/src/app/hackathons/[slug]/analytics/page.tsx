...
export default function HackathonAnalyticsPage() {
    ...
    useEffect(() => {
        const fetchAnalytics = async () => {
            if (!slug) {
                // console.error("Frontend DEBUG (Analytics Page): Slug is undefined in useParams.");
                return;
            }

            if (!isLoaded || !isSignedIn || !user) {
                // console.warn("Frontend DEBUG (Analytics Page): User not loaded or not signed in. Skipping fetchAnalytics.");
                setLoading(false);
                return;
            }
            ...
            try {
                ...
                // console.error('Frontend DEBUG (Analytics Page): Step 1 API call (basic info) to:', basicHackathonApiUrl);
                ...
                // console.error(`\nFrontend DEBUG (Analytics Auth): Current user.id (Clerk ID): "${currentUserClerkId}"`);
                // console.error(`Frontend DEBUG (Analytics Auth): Hackathon creator.clerkId: "${creatorClerkId}"`);
                // console.error(`Frontend DEBUG (Analytics Auth): user.id === creator.clerkId? ${currentUserClerkId === creatorClerkId}`);

                const isCreator = currentUserClerkId === creatorClerkId;
                const isExpert = user.publicMetadata?.role === 'EXPERT';
                const isAdmin = user.publicMetadata?.role === 'ADMIN';

                // console.error(`Frontend DEBUG (Analytics Auth): isCreator: ${isCreator}`);
                // console.error(`Frontend DEBUG (Analytics Auth): isExpert (from publicMetadata): ${isExpert}`);
                // console.error(`Frontend DEBUG (Analytics Auth): isAdmin (from publicMetadata): ${isAdmin}`);
                ...
                if (isExpert && hackathonCompanyId && userPublicMetadataCompanyId) {
                    // console.error(`Frontend DEBUG (Analytics Auth): User publicMetadata companyId: "${userPublicMetadataCompanyId}"`);
                    // console.error(`Frontend DEBUG (Analytics Auth): Hackathon companyId: "${hackathonCompanyId}"`);
                    // console.error(`Frontend DEBUG (Analytics Auth): Company IDs matching? ${userPublicMetadataCompanyId === hackathonCompanyId}`);
                    // Note: If these are CUIDs, they should directly compare. If one has invisible chars, backend .trim() is for server, this might need it too.
                    isCompanyExpert = userPublicMetadataCompanyId === hackathonCompanyId;
                }
                // console.error(`Frontend DEBUG (Analytics Auth): isCompanyExpert: ${isCompanyExpert}`);
                // console.error(`Frontend DEBUG (Analytics Auth): Overall Check (!isCreator && !isAdmin && !isCompanyExpert): ${!isCreator && !isAdmin && !isCompanyExpert}`);


                if (!isCreator && !isAdmin && !isCompanyExpert) {
                    setError('Unauthorized to view analytics for this hackathon.');
                    setLoading(false);
                    // console.warn("Frontend DEBUG (Analytics Page): User unauthorized based on client-side check. Returning early.");
                    return;
                }

                const analyticsApiUrl = `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/hackathons/${fetchedHackathonId}/analytics`;
                // console.error('Frontend DEBUG (Analytics Page): Step 2 API call (analytics data) to:', analyticsApiUrl);
                ...
                // console.error('Frontend DEBUG (Analytics Page): Failed to fetch analytics (catch block):', err);
                if (err.response?.status === 403) {
                    setError('You do not have permission to view analytics for this hackathon.');
                } else {
                    setError(err.response?.data?.error || 'Failed to load analytics. Check network and backend logs.');
                }
            } finally {
                setLoading(false);
            }
                                    ...
                                    <tr key={data.id}>
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                            <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${
                                                data.submissionStatus === 'PENDING REVIEW' ? 'bg-yellow-100 text-yellow-800' :
