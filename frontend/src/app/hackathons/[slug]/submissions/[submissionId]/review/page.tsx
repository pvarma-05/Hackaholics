    ...
    const onSubmitReview = async (data: ReviewFormData) => {
        ...
        try {
            ...
            const response = await axios.patch(
                `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/submissions/${submission.id}/review`,
                data, 
                { headers: { Authorization: `Bearer ${token}` } }
            );
            console.log('Submission reviewed:', response.data);
            
            setSubmission(prev => prev ? { ...prev, ...response.data, status: 'REVIEWED_MANUAL' } : null);
    ...
    if (error) {
        return (
            <div className="min-h-screen flex flex-col items-center justify-center bg-gray-100 p-4">
                ...
                <Link href={`/hackathons/${slug}/submissions`} className="text-yellow-300/70 hover:underline font-medium">
                    Back to Submissions
                </Link>
            </div>
        );
    ...
    return (
        ...
        >
            ...
            <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
                <Link href={`/hackathons/${slug}/submissions`} className="flex items-center text-gray-700 hover:text-gray-900 gap-2 mb-6">
                    <IconArrowLeft size={20} /> Back to All Submissions
                </Link>
