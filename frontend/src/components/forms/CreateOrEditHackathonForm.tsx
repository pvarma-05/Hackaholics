...
interface CreateOrEditHackathonFormProps {
    initialData?: HackathonFormData; 
    hackathonId?: string; 
}

export function CreateOrEditHackathonForm({ initialData, hackathonId }: CreateOrEditHackathonFormProps) {
  ...
  useEffect(() => {
    const fetchExpertCompany = async () => {
      ...
      try {
        ...
        if (profileData.role === 'EXPERT' && profileData.company && profileData.isApprovedInCompany) {
            ...
            if (!hackathonId) { 
                setValue('companyId', profileData.company.id);
            }
        } else {
    ...
    fetchExpertCompany();
  }, [user, getToken, hackathonId, setValue]);
  ...
  const onSubmit = async (data: HackathonFormData) => {
    ...
    const companyIdToSubmit = hackathonId ? (initialData?.companyId || null) : (expertAssociatedCompany?.id || null);
    ...
    try {
      ...
      if (imageFile) {
        ...
        finalBannerImageUrl = uploadResponse.data.url;
          ...
          finalBannerImageUrl = hackathonId ? initialData?.bannerImageUrl : undefined;
      }
      ...
      const token = await getToken();
      const apiUrl = hackathonId ? `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/hackathons/${hackathonId}` : `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/hackathons`;
      const requestMethod = hackathonId ? axios.patch : axios.post;
      ...
      console.log('Hackathon saved:', response.data);
      router.push(`/hackathons/${response.data.slug}`);
    } catch (err: any) {
      console.error('Error saving hackathon:', err);
      setFormError(err.response?.data?.error || 'Failed to save hackathon. Please try again.');
    } finally {
      setSubmitLoading(false);
    }
  };
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-100 p-4">
      ...
      >
        <h2 className="text-3xl font-poppins font-semibold text-gray-900 mb-6">
          {hackathonId ? 'Edit Hackathon' : 'Create New Hackathon'}
        </h2>
        ...
        <div>
          ...
          />
          {hackathonId && initialData?.bannerImageUrl && (
            <p className="mt-2 text-xs text-gray-500">
              Current: <a href={initialData.bannerImageUrl} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline">View Current Image</a> (Upload new to replace)
            </p>
          )}
        ...
        >
          {submitLoading ? (
            <>
              ...
              {hackathonId ? 'Saving Changes...' : 'Creating Hackathon...'}
            </>
          ) : (
            hackathonId ? 'Save Changes' : 'Create Hackathon'
          )}
        </button>
      </form>
    </div>
